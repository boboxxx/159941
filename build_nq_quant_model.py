...
def merge_asof_features(
    ...
    merged = pd.merge_asof(
        ...
        right.sort_values(right_date_col),
        left_on="date",
...
def summarize_live_drivers(pipeline: Pipeline, row: pd.DataFrame, feature_cols: list[str], top_n: int = 5) -> str:
    ...
    return " | ".join(f"{feat}:{val:+.3f}" for feat, val in zip(driver["feature"], driver["contribution"], strict=True))
...
def plot_backtest(us_art: ModelArtifacts, cn_art: ModelArtifacts, cn_intraday: IntradayArtifacts, output_path: Path) -> None:
    ...
    for ax, art, title in zip(axes[:2], [us_art, cn_art], ["US QQQ Daily", "CN Nasdaq ETF Daily"], strict=True):
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--events", default="wsquant_market_timeline.xlsx", help="Path to the signal timeline workbook.")
    parser.add_argument("--output", default="nq_quant_model_results.xlsx", help="Excel output path.")
    parser.add_argument("--plot", default="nq_quant_model_backtest.png", help="Backtest plot path.")
    parser.add_argument("--live-output", default="nq_live_signals.csv", help="CSV output path for next-session live signals.")
    ...
    parser.add_argument("--nq-archive-dir", default="nq_contract_archive", help="Directory for archived NQ single-month contracts.")
