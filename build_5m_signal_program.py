...
def build_signal_summary(
    ...
    summary = (
        ...
        f"NQMAIN_CLOSE({price_format('NQ=F', nqmain_close_value)}) "
...
def latest_nqmain_close_value(nq_raw: pd.DataFrame) -> float:
    ...
    latest_nonzero = latest[latest["volume"].fillna(0) > 0]
    if not latest_nonzero.empty:
        return float(latest_nonzero.iloc[-1]["close"])
    return float(latest.iloc[-1]["close"])
...
def make_intraday_dataset(
    ...
    base["trade_ret_1"] = trade_ret_1
    base["trade_ret_h"] = trade_ret_h
    base["fwd_low_dd_h"] = future_low / close - 1.0
    base["fwd_high_up_h"] = future_high / close - 1.0
    base["target"] = ((edge > profit_threshold) & (base["fwd_low_dd_h"] > -stop_loss)).astype(float)
    base["current_state"] = 0
    base["target_time"] = base["datetime"].shift(-1)
...
def apply_5m_overlay(probability: float, row: pd.Series, market: str) -> tuple[float, str]:
    ...
    if all(pd.notna(x) for x in (cnetf_ew, nq_ew, cnetf_fib)):
        ...
        if cnetf_fib < 0.20 and cnetf_ew > -0.25 and nq_ew > -0.30:
            return max(adjusted, 0.60), "cn_intraday_rebound"
    return adjusted, reason
...
def live_row_from_frame(
    ...
    latest = live_frame.dropna(subset=art.feature_cols).sort_values("datetime").tail(1)
    ...
    probability, overlay_reason = apply_5m_overlay(raw_probability, latest.iloc[0], market)
    recommended_state = current_state
    if current_state == 0 and probability >= entry_threshold:
        recommended_state = 1
    ...
    return {
        ...
        "wave_overlay": overlay_reason,
        "nqmain_close_value": float(nqmain_close_value),
        "signal_price": float(latest["close"].iloc[0]),
        "trade_date": pd.Timestamp(latest["trade_date"].iloc[0]),
...
def main() -> None:
    ...
    nqmain_close_value = latest_nqmain_close_value(nq_raw)
    ...
    if need_us:
        ...
        us_art = walk_forward_5m(us_dataset, "美股5m", entry_threshold=0.57, exit_threshold=0.40, stop_loss=0.004)
        ...
        live_rows.append(live_row_from_frame(us_art, us_live_features, "美股", "QQQ", prior_row, 0.57, 0.40, nqmain_close_value))
    ...
    if need_cn:
        ...
        cn_art = walk_forward_5m(cn_dataset, "场内5m", entry_threshold=0.55, exit_threshold=0.40, stop_loss=0.006)
        ...
        live_rows.append(live_row_from_frame(cn_art, cn_live_features, "场内", args.cn_symbol, prior_row, 0.55, 0.40, nqmain_close_value))
